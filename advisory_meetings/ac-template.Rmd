...
knitr::opts_chunk$set(
  ...
  include = FALSE  # Suppress code print out
)
```

```{r libraries}
library(tidyverse) # For data manipulation packages
library(DBI) # For connecting to databases
library(ROracle) # For connecting to databases
```
...
repeat {
  loc <- readline(prompt = "Select a location (YS, QL, GR, SWNB): ")
...
```{r reporting_rate}
# Need more information on what licences exist and where they exist before 
# we can create something standardised here
```

# ESCAPEMENT (YS & GR ONLY)
```{r lake_vaughan_escapement}
# Connect to GASPEREA db for this part
oracle.password.GASP <- "_REMOVED"
...
if (loc == "YS") {
  ...
  escaped |> 
    ...
    geom_hline(yintercept = 2.1, linetype = "dotted", size = 3/5) +
    annotate("text", x = 5.5, y = 2.95,  label = "Healthy", size = 5, alpha = 0.7, colour = "green4") +
    annotate("text", x = 5.5, y = 2.35,  label = "Cautious", size = 5, alpha = 0.7, colour = "orange2") +
    annotate("text", x = 5.5,  y = 1.55,  label = "Critical", size = 5, alpha = 0.7, colour = "red4") +
    labs(
