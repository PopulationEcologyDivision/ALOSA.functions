...
require(knitr)
require(xtable)
##source Mark's function

source("R:/Science/Population Ecology Division/DFD/Alosa/Tusket Tracking/Scripts/VR2WDataCleaner1.1.R")

##Run function with correct metadata stuff

CleanVR2WData(data.dir = "R:/Science/Population Ecology Division/DFD/Alosa/Tusket Tracking/Receiver Downloads",
              receiver.meta.path = "R:/Science/Population Ecology Division/DFD/Alosa/Tusket Tracking/OTN_format_2021_Tusket_Receivers.csv",
              tags.meta.path = "R:/Science/Population Ecology Division/DFD/Alosa/Tusket Tracking/OTN_format_2021_Tusket_Tags.csv",
              threshold = 3,
              dataset.name = "tusk21")

####Fix tag and rec meta data date time cols####
#tusk21.VR.tag.meta$UTC_RELEASE_DATE_TIME<-gsub("T"," ",tusk21.VR.tag.meta$UTC_RELEASE_DATE_TIME)
#tusk21.VR.tag.meta$UTC_RELEASE_DATE_TIME<-as.POSIXct(tusk21.VR.tag.meta$UTC_RELEASE_DATE_TIME,tz="UTC")
tusk21.VR.rec.meta$DEPLOY_DATE_TIME....yyyy.mm.ddThh.mm.ss.<-gsub("T"," ",tusk21.VR.rec.meta$DEPLOY_DATE_TIME....yyyy.mm.ddThh.mm.ss.)
tusk21.VR.rec.meta$DEPLOY_DATE_TIME....yyyy.mm.ddThh.mm.ss.<-as.POSIXct(tusk21.VR.rec.meta$DEPLOY_DATE_TIME....yyyy.mm.ddThh.mm.ss.,tz="UTC")
tusk21.VR.rec.meta$RECOVER_DATE_TIME..yyyy.mm.ddThh.mm.ss.<-gsub("T"," ",tusk21.VR.rec.meta$RECOVER_DATE_TIME..yyyy.mm.ddThh.mm.ss.)
tusk21.VR.rec.meta$RECOVER_DATE_TIME..yyyy.mm.ddThh.mm.ss.<-as.POSIXct(tusk21.VR.rec.meta$RECOVER_DATE_TIME..yyyy.mm.ddThh.mm.ss.,tz="UTC")
...
#compare all tags detected to all tags released, if there are any detected that aren't in metadata

# for(i in 1:length(good.data$tag.id.code)){
#   good.data$tag.id.code[i] 
# }
...
setwd("C:/Users/naug/Documents/R/GIS/Tusket R/Shapefiles/NHN")
...
points <- st_as_sf(recs[1:7,], coords = c("DEPLOY_LONG", "DEPLOY_LAT"), remove = F, crs = 4326)
...
ggplot()+
  ...
  geom_text_repel(data = recs[1:7,], aes(x = DEPLOY_LONG, y = DEPLOY_LAT, label = OTN_ARRAY), size = 3)+
  geom_text(data = watbodcoords, aes(X, Y, label = LAKENAME_1), colour = "steelblue4", size = 2)+
  coord_sf(xlim = c(-66, -65.84), ylim = c(43.85, 43.94), datum = NA, expand = FALSE, clip = "off")+
  theme_minimal()+
...
table3 <- fulldet %>%
  group_by(OTN.array, SP)%>%
  summarize("Number of Tags" = length(unique(tag.id.code)))%>%
  rename("Receiver Station Name" = OTN.array, Species = SP)
...
table4 <- end.rec.df2%>%
  group_by(Last.OTN.array, SP)%>%
  summarize("Number of Tags" = length(unique(tag.id.code)))%>%
  rename("Last Receiver" = Last.OTN.array, "Species" = SP)
```
...
tag.meta.sub<-tusk21.VR.tag.meta[,c(5,6,15,30,37)] ##grab some tag meta data, maybe add more later
...
n.released<-nrow(tusk21.VR.tag.meta) ##total tags released



tusk21.VR.tag.meta.A<-tusk21.VR.tag.meta[tusk21.VR.tag.meta$COMMON_NAME_E=="Alewife",]
n.A.released<-nrow(tusk21.VR.tag.meta.A) ##total A released
A.det<-intersect(tusk21.VR.tag.meta.A$TAG_ID_CODE,tags.detected)
...
##BB time
tusk21.VR.tag.meta.B<-tusk21.VR.tag.meta[tusk21.VR.tag.meta$COMMON_NAME_E=="Blueback Herring",]
n.B.released<-nrow(tusk21.VR.tag.meta.B) ##total B released
B.det<-intersect(tusk21.VR.tag.meta.B$TAG_ID_CODE,tags.detected)
