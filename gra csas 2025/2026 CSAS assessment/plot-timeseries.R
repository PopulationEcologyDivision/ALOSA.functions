...
fsar_plot_base <- function(in.df, language = c("English","French")) {
  ...
  yl <- c(0, max(in.df[idx, "ts.value"], na.rm=T) * 1.3)
  # yl <- c(0, 500) #hardcode to match ssb plot
  tl.df <- in.df[idx, ]

  if (language == "English") {
    x.lab <- "Year"
    y.lab <- "Catch"
    legend.text <- c("Catch (t)")
  }
  if (language == "French") {
    x.lab <- "Année"
    y.lab <- "Capture"
    legend.text <- c("Capture (t)")
  }

  plot(ts.value ~ year, data = tl.df, type = "n", axes = FALSE, xlab = "", ylab = "", ylim = yl)
  ## Catch
  lines(ts.value ~ year, data = tl.df[which(tl.df$ts.name == "Catch"), ], type = "l", lwd = 2)
  ## TAC
  ...
  yl <- c(0, max(in.df[idx, "ts.value"], na.rm=T) * 1.4)
  # yl < -c(0, 500) #hardcode to match catch plot
  tr.df <- in.df[idx, ]

  if (language == "English") {
    y.lab <- "Biomass"
    legend.text <- c("SSB (t)", "95% confidence", "USR (t)", "LRP (t)")
  }
  if (language == "French") {
    y.lab <- "Biomasse"
    legend.text <- c("BSR (t)", "confiance à 95%", "NRS (t)", "NRL (t)")
  }
  ...
  if (language == "English") {
    y.lab <- "Mortality"
    legend.text <- c("Exploitation", "95% confidence", "RR", "TRR", "M")
  }
  if (language == "French") {
    y.lab <- "Mortalité"
    legend.text <- c("F-1/yr", "confiance à 95%", "RR", "TRR", "M")
  }

  idx <- which(in.df$panel.category == "Fishing" & in.df$ts.name == "µ")
  plot(ts.value ~ year, data = bl.df[which(bl.df$ts.name == "µ"), ], type = "n", axes = FALSE, xlab = "", ylab = "", ylim = yl)
  ...
  yl <- c(0, max(in.df[idx, "ts.value"], na.rm=T) * 1)
  br.df <- in.df[idx, ]

  if (language == "English") {
    y.lab <- "Recruitment"
    legend.text <- c("Age-3 Recruits (millions of fish)", "95% confidence")
  }
  if (language == "French") {
    y.lab <- "Recrutement"
    legend.text <- c("R-E06", "confiance à 95%")
  }
  plot(ts.value ~ year, data = br.df[which(br.df$ts.name == "Age-3 Recruits"), ], type = "n", axes = FALSE, xlab = "", ylab = "", ylim = yl)
   #lines(ts.value ~ year, data = br.df[which(br.df$ts.name == "Rlow"), ], type = "l", lty = 2)
   #lines(ts.value ~ year, data = br.df[which(br.df$ts.name == "Rhigh"), ], type = "l", lty = 2)
  #get rid of Na rows
  br.df<-br.df[complete.cases(br.df),]
  br.df.low <- br.df[which(br.df$ts.name == "Age-3 Recruits Low"), ]
  br.df.high <- br.df[which(br.df$ts.name == "Age-3 Recruits High"), ]
  ## use grey shading polygons instead of lines
  polygon(c(br.df.low$year, rev(br.df.high$year)), c(br.df.low$ts.value, rev(br.df.high$ts.value)), col=grey(0.8), border=grey(0.8))
  lines(ts.value ~ year, data = br.df[which(br.df$ts.name == "Age-3 Recruits"), ], lwd = 2)
