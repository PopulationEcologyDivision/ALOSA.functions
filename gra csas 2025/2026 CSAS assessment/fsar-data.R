...
org_fsar_data <- function(format = c("wide", "long")) {
  ...
  ## precipitated R
  ts4 <- c(rec.plot,NA,NA)
  ts4.lo<-c(rec.plot.lower,NA,NA)
  ts4.hi<-c(rec.plot.upper,NA,NA)
  ...
  sim.df <-
    rbind(
      ...
      data.frame(
        ...
        year = rep(years, 3),
        ts.name = rep(c("Age-3 Recruits", "Age-3 Recruits Low", "Age-3 Recruits High"), each = length(years)),
