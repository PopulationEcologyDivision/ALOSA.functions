The purpose of this markdown file is to have a place where we can:
...
```{r source_ALOSA_functions, warning=FALSE}
...
```{r escapement_vd, warning=FALSE, message=FALSE}
...
```{r escapement_ph, message=FALSE, warning=FALSE}
# Locate your CSV for counts
...
out <- data.frame(day.int = 1:nrow(pp), total_fish = rep(1,nrow(pp)))
out.pred <- predict(fit, newdata = out, type = "link", se.fit = T)

# This is the inverse of the link function i.e. the inverse of the log i.e exp
ginv <- fit$family$linkinv
...
pp |> 
  ...
  geom_path(colour = "grey50", size = 0.5, linetype = "dashed")+
