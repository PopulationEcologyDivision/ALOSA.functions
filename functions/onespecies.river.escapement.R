...
onespecies.river.escapement <- function(
    ...
    if(database == F){
      ...
      if(fixtime == T){goodnames = (c("year", "mon", "day", "time", "strata", "count.upstream", "count.downstream", "minutes", "seconds"))}
    ...
    if(downstream.migration == T){count.data$total <- ifelse(count.data$total < 0, 0, count.data$total)}
    
    if(fixtime == T){count.data$total = round((count.data$total / (count.data$minutes * 60 + count.data$seconds)) * 300)}
    
    # Date conversion amalgamates month and year columns into one format
    count.data$date = as.Date(paste(count.data$day, count.data$mon, count.data$year, sep = "-"), format = "%d-%m-%Y")
    ...
    start.end = first.last.day(count.data, "dayofyear", count.data$year[1], "total")
    
    # Select only data from day of first count onwards
    count.data = count.data[count.data$dayofyear >= start.end[1] & count.data$dayofyear <= start.end[2], ]
    
    # --- MEANS, STANDARD DEVIATION, VARIANCE, STANDARD ERROR AND TIME UNITS 
    # SAMPLES PER PERIOD PER DAY (two way stratified) ---
    
    # 1A: Mean Counts
    strata.means <- aggregate(
      count.data$total,
      by = list(count.data$dayofyear, count.data$strata),
      FUN = "mean",
      na.rm = T
      )
    ...
    alldays = data.frame(
      Group.1 = rep(
        ...
        each = length(n.strata)
        ),
      Group.2 = rep(
        n.strata,
        times = length(start.end[1]:start.end[2])
        )
      )
    ...
    if(length(alldays$n.counts[is.na(alldays$n.counts) | alldays$n.counts <= 1]) > 0 ){
      ...
      # fill in missing mean count, n.counts, sample.var, and sd for missing strata.
    ...
    #if(n.strata==6){print(paste("Strata 6 total:",strata.percent(6,summary.data)))}
    ...
    # Satterthwaite approximation of degrees and freedom (formula from Nelson 2006)
    # This is a work around for counts strata with only a single count, 
    # which prevents the df calculation and therefore the total CI calculation.
    summary.data.df <- summary.data[summary.data$n.counts > 1, ] 
    ...
    # 1C: Daily Variance 
    summary.data$daily.variance.term <- runVAR.term(summary.data)
    
    daily.var=aggregate(
      summary.data$daily.variance.term,
      by = list(summary.data$dayofyear),
      FUN = "sum",
      na.rm = T
      )
    ...
    for(i in start.end[1]:start.end[2]){
      daily.summary$df1[daily.summary$dayofyear==i] <-
        satterthwaite.approx.df(
          summary.data[summary.data$dayofyear == i, ]
          )
    }
    
    # Set alpha value and find critical value
    alpha <- 0.05
    daily.summary$clow <- daily.summary$total - qt(1 - alpha / 2, daily.summary$df1) * sqrt(daily.summary$var)
    daily.summary$chigh <- daily.summary$total + qt(1 - alpha / 2, daily.summary$df1) * sqrt(daily.summary$var) 
    ...
    daily.summary <- (daily.summary[ , c("mon", "day", "dayofyear", "total", "sd", "clow", "chigh")])
    
    print(
      paste(
        "First Fish on:",
        paste(
          daily.summary$mon[which(daily.summary$total > 0)[1]],
          daily.summary$day[which(daily.summary$total > 0)[1]],
          sep = "-"
          )
        )
      )
    ...
    print(
      paste(
        "Last Fish on:",
        paste(
          daily.summary$mon[max(which(daily.summary$total > 0))],
          daily.summary$day[max(which(daily.summary$total > 0))],
          sep = "-"
          )
        )
      )
    ...
    print(
      paste(
        "Peak of Run on:",
        paste(
          daily.summary$mon[daily.summary$total==max(daily.summary$total)],
          daily.summary$day[daily.summary$total==max(daily.summary$total)],
          sep = "-"
          ),
        "with",
        max(daily.summary$total),
        "fish"
        )
      )
