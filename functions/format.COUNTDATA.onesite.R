...
format.COUNTDATA.onesite <- function(filename){
  ...
  goodnames <- (c("year", "mon", "day", "time", "strata", "count.upstream", "count.downstream", "camera.desc", "notes", "minutes", "seconds"))
  ...
  for(i in 1:dim(data)[1]){
    ...
    if(is.na(data$count.upstream[i]) & !is.na(data$count.downstream[i])){ data$count.upstream[i] = 0 }
    
    if(!is.na(data$count.upstream[i]) & is.na(data$count.downstream[i])){ data$count.downstream[i] = 0 }
    
  }
  
  col_order <- c("COUNT_ID", "SITE_ID", "year", "mon", "day","camera.desc", "strata", "time", "count.upstream", "count.downstream", "notes", "minutes", "seconds")
  
  data <- data[ , col_order]
  
  names(data) <- c("COUNT_ID", "SITE_ID", "YEAR", "MON", "DAY","CAMERA_DESC", "STRATA", "TIME", "COUNT_UP", "COUNT_DOWN", "NOTES", "MINUTES", "SECONDS") 
