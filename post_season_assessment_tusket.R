...
post_season_assessment_tusket <-
  function(year = NA,
    ...
    if (powerhouse == TRUE) {
      ...
      vd_file <- dbReadTable(channel, "ALOSA_VIDEO_COUNT_DATA") |>
        filter(YEAR == year & SITE_ID == 2)
      
      message(paste0("Getting count data for Powerhouse for ", year))
      ph_file <- dbReadTable(channel, "ALOSA_VIDEO_COUNT_DATA") |>
        filter(YEAR == year & SITE_ID == 14)
      ...
      vd_counts <- onespecies.river.escapement(
        vd_file,
        fixtime = TRUE,
      ...
      vd_counts$year <- as.character(year)
      vd_counts$location <- "Lake Vaughan"
      ...
      if (year >= 2024) {
        ...
        on.exit(sink(), add = TRUE)  # this prevents info from being printed to console
        ph_counts <- onespecies.river.escapement(
          "R:/Science/Population Ecology Division/DFD/Alosa/Locations/Tusket River/Tusket 2024/Powerhouse 2024 count data.csv",
          fixtime = TRUE,
          downstream.migration = FALSE,
          database = FALSE,
          2024,
      ...
      counts <- rbind(vd_counts, ph_counts)
      
    }
    
    ## PH = FALSE ####
    ## If no data are desired for PH, then we can just work with VD
    if (powerhouse == FALSE) {
      message(paste0("Getting count data for Vaughan Dam for ", year))
      vd_file <- dbReadTable(channel, "ALOSA_VIDEO_COUNT_DATA") |>
        filter(YEAR == year & SITE_ID == 2)
    ...
    proportions <- split_species(
      database = TRUE,
      biodata_file = NA,
      accessory_data = if_else(year >= 2022, TRUE, FALSE),
      accessory_file = NA,
      site_id = 2,
      year = year
    )
