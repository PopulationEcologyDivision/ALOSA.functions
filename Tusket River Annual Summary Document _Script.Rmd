...
header-includes:
   ...
   - \fancyhead[L]{Tusket River 2025 Assessment Summary}
   - \fancyfoot[R]{\thepage}
...
-----|---------|----------|----------|--------|--------|
2026    | 1,072,187 | NA        | NA      | NA      | NA 
2025    | 1,372,982 | 1,371,169 | 1,813   | 0.22    | 0.26  
2024    | 2,262,181 | 2,252,147 | 10,034  | 0.44    | 0.29
2023    | 2,119,723 | 2,101,735 | 17,988  | 0.43    | 0.16
2022    | 1,670,471 | 1,662,008 | 8,463   | 0.44^g^ | 0.25^h^ 
...
reference.point.plot<-function(
  ...
  mtext(paste("Esc USR =", ssnmsy, sep = ""), 1, line = 6, at = 1.5, las = 0, adj = 1)
...
series24.25 = TR[TR$year >= 2024 & TR$year <= 2025, ]
series25    = TR[TR$year == 2025, ]
...
points(series24.25$escape / ssnmsy, series24.25$er / Fupper, pch = 1)
lines(series24.25$escape / ssnmsy, series24.25$er / Fupper)

points(series25$escape / ssnmsy, series25$er / Fupper, pch = 19) # colored point
lines(series25$escape / ssnmsy, series25$er / Fupper)
...
text(0.60, 0.86, "2023", pos=4, col = "gray35", cex = 0.85)
text(0.73, 0.85, "2024", pos=4, col = "gray35", cex = 0.85)
text(0.4, 0.26, "2025", pos=4, col = "gray35", cex = 0.85)

```

**Figure 2.** Stock status plot for the Tusket River Alewife stock. Each point indicates status in a single year, and the lines connect the points in sequence. Stock status for 2014 - 2024 are shown using open circles, while status from 2025 is shown with a closed circle. The plot is updated from Gibson et al. 2017.
...
data$doy = as.Date(paste(data$day, data$mon, data$year, sep = "-"), format = "%d-%m-%Y")
data$doy = as.numeric(strftime(data$doy, format = "%j"))
TRdata = data[data$river == "tusket" | data$river == "Tusket", ]

###gotta reassign this to get blueback numbers in
TR = multiyear[multiyear$river == "Tusket" | multiyear$river == "tusket", 1:9]

plotyears = c(2023, 2024, 2025)
par(oma = c(7, 3, 3, 0.5), mfrow = c(3, 1))
par(mar = c(0, 5, 0, 3))
